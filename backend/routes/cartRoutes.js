...
router.post("/",async (req, res)=>{
    ...
    //   Determne if the user is logged in or guest
    ...
    return res.status(201).json(newCart);
      ...
      console.error(error);
      res.status(500).json({ message: "Server error in cart"})
    }
})
    ...
    try{
        ...
        if(guestCart){
            ...
            if(userCart){
                ...
                // romove the guest cart after merging 
                try{
            ...
            res.status(404).json({ message: "Guset cart not found"});
        }
    }catch(error){
    console.log(error);
    res.status(500).json({message: "Server Error"});
